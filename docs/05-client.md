...
`node.state.<name>.subscribe(handler)` registers a listener for a `state.publish` (or `buffer.publish`) slot. The runtime watches the slot's per-slot version counter (see `02-messaging.md` §5.4 and `04-worklet-runtime.md` §7 for the due-tick scheduler). On each due tick where the version has advanced, the runtime reads the value and invokes the handler — including when the new value is identical to the previously delivered one. **The framework does not perform value-equality dedupe** (Q39-b, `decisions-log.md`): the audio thread increments the version counter unconditionally every due tick, and the main side delivers every advance.
...
`07-unplugin.md` documents the Vite-specific delivery details (the `?worklet` import shape, the HMR boundary the plugin sets up). The swap orchestration itself stays in user code.
