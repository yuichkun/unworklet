...
### code が guide と食い違う finding の扱い

`skills/unworklet/` は実装と突き合わせ済みの説明面だが、それでも**基準は方向性**であって guide の文字面ではない (`../../_shared/core-principles.md` §1)。方向性に照らして振り分ける:

- code が**方向性に反している** → A (guide に関係なく code を直す)
- **guide の方が古い**で code が方向性に沿っている → A (guide を直す。放置は次の consumer が踏むので nit ではない)
- **どちらが正か方向性だけでは決まらない** (解釈が割れる) → B で聞く

どちらに倒すにしても、**正本を 1 つに保つ**のが目的。両方残して食い違わせるのが最悪。なお設計時の spec 文書群は削除済みなので、「spec と違う」という finding 分類は存在しない。
