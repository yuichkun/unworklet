...
`state.buffer.<T>` publish の T 制 限: `state.<type>` publish と 別 ル ー ル で、 **全 element type で 受 容** (= `state.buffer.f32` / `state.buffer.f64` / `state.buffer.i32` / `state.buffer.i64` / `state.buffer.bool` / `state.buffer.u8` 全 部 publish 可)。 region 全 体 が memcpy 経 由 で 1 atomic op 制 約 ナ シ (= 上 記 torn-read OK 設 計)。 main 側 subscriber の handler 引 数 型 は declaration kind ご と に narrow (= `state.buffer.f32` publish → `Float32Array`、 `state.buffer.f64` → `Float64Array`、 `state.buffer.i32` → `Int32Array`、 `state.buffer.i64` → `BigInt64Array`、 `state.buffer.bool` → `Uint8Array` (0/1)、 `state.buffer.u8` → `Uint8Array`)。
