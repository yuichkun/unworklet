...
import { myProcessor } from "./my-processor"; // Import the return value of `defineProcessor(...)` (i.e. `CompiledProcessor<C>`) directly. The `?worklet` import path via unplugin also works in parallel — as long as `defineProcessor(...)` is a named export inside the `.processor.ts` file, either import path yields the same artifact.
