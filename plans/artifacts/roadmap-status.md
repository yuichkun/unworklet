...
余湖さん's hardware). Run from the repo root.

| item   | what                                                    | status | evidence                                                                                                                                                                                                                         |
| ------ | ------------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A1** | 4 packages + internal modules `vp build` exit 0         | ✓      | CI "Build packages" step (`vp run --filter @unworklet/{vite-plugin,core,lang,offline,test} build`) — green on the PR                                                                                                             |
...
| **D1** | browser matrix Chromium × FF × Safari × {isolated, not} | ◐      | **Chromium ✓** — CI "Vitest (browser SAB + postMessage)" + "Playwright e2e" green; **Firefox / Safari pending** — needs 余湖さん's hardware (no webkit/gecko runner in CI; `packages/core/vite.browser*.config.ts` are Chromium) |
...
real browsers on 余湖さん's machine) — Chromium is fully covered. v1.0.0 is
