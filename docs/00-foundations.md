...
The body of `defineProcessor` and `defineSubgraph`, before the returned `process` lambda. The only place where new declarations are created: `state.*`, `state.buffer.*`, `param.*`, `audioInput`, `audioOutput`, `event` (with `from` / `to` direction, plus `event.midi` for MIDI ports), and subgraph instantiations via `instantiate(...)`. Each declaration registers a slot in the graph and a region in WASM linear memory.
