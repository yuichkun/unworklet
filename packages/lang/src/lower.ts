...
export function lower(source: string, options: LowerOptions = {}): string {
  ...
  const declaredNames = statementBoundNames(declarations);
  const reserved = [...generatedBindings].find((name) => declaredNames.has(name));
  if (reserved !== undefined) {
    throw new LowerError(
      "uwk-reserved-binding",
      `\`${reserved}\` is generated by the lowering in this file, so a declaration of that ` +
        `name would take its place — rename the binding. (The ambient \`input\` / \`out\` and ` +
        `their factories are only generated when the file declares no audio I/O of its own.)`,
    );
  }
