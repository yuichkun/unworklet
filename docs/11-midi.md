...
A processor declares MIDI involvement explicitly in declaration scope. MIDI is part of the `event` family, discriminated by direction: `event.midi({ from: 'main' })` declares ingestion (device → worklet) and `event.midi({ to: 'main' })` declares emission (worklet → device). Either can be omitted. A processor that calls neither has no MIDI surface at all (the concepts are absent from its API and IDE completion).

```typescript
event.midi(options: { from: "main"; name: string; capacity?: Capacity }): MidiInputHandle;
event.midi(options: { to:   "main"; name: string; capacity?: Capacity }): MidiOutputHandle;
```

```typescript
const synth = defineProcessor((ctx) => {
  const midiIn = event.midi({ from: "main", name: "midiIn" }); // declare MIDI ingestion
  // ...
  return {
    process: () => {
      /* ... */
...
const arpeggiator = defineProcessor((ctx) => {
  ...
  const midiOut = event.midi({ to: "main", name: "midiOut" }); // declare MIDI emission
  // ...
});
...
const dualPort = defineProcessor((ctx) => {
  ...
  const sync = event.midi({ from: "main", name: "sync" }); // external clock / transport
  const arpOut = event.midi({ to: "main", name: "arpOut" }); // arp-generated notes
  // ...
});

const audioOnly = defineProcessor((ctx) => {
  // No event.midi(...) call → MIDI concepts unused, never appear in this processor's surface.
});
```

Options:

- **`from: 'main'` / `to: 'main'`** — required direction discriminator. `from: 'main'` is ingestion (device → worklet) and yields a `MidiInputHandle`; `to: 'main'` is emission (worklet → device) and yields a `MidiOutputHandle`. MIDI lives on the main thread (Web MIDI), so both directions are bridged through main — the discriminator names which way events flow relative to the worklet.
- **`name: string`** — required. Used as the identifier in three places: (a) main-side access — `node.midi.<name>` resolves to this declaration's surface (`send` / `connectFromWebMIDI` / `onEvent` / `diagnostics.overflowCount`); (b) snapshot schema-hash identity — declarations participate in the structural hash that drives migration matching, even though the MIDI ringbuffer state itself is transient; (c) error and diagnostic messages — `event.midi 'sync' overflowed: 12 events dropped` is more actionable than `event.midi[0] overflowed`. Required for the same reasons as `name` on `state` / `state.buffer` / `param` / `event` declarations: the framework refuses index-based identity to keep declaration order non-load-bearing.
...
The exact shape of the `event.midi({ from: 'main' })` / `event.midi({ to: 'main' })` handles (event subscription on the worklet side, emission primitives, etc.) is settled in §2 and §4.

## 2. Sample-accurate event handling

### 2.1 Inbound: subscribing to MIDI events (worklet side)

`event.midi({ from: 'main' })` returns a handle whose primary surface is `onEvent(eventType, handler)`. Handlers are **type-discriminated**: each registered handler observes one event type, and TypeScript narrows the handler's argument shape accordingly.
...
MIDI events appear in two contexts: **main thread** (= `node.midi.<name>.send(...)` argument and `.onEvent(...)` handler argument), and **worklet audio-thread** (= `event.midi({ from: 'main' }).onEvent(...)` handler argument and `event.midi({ to: 'main' }).emitIf(...)` argument). Each context uses a distinct TypeScript type because the values themselves are different things — main-thread fields are plain JS numbers, worklet fields are audio-graph nodes captured into the WASM emission (Q22). Authors never see raw status bytes — the compiler generates serializers and deserializers between these types and the wire format (§4).
...
// proxy (Q36-b). Returned by `event.midi({ from: 'main' }).onEvent(...)` handler,
// and accepted by `event.midi({ to: 'main' }).emitIf(...)`. See `decisions-log.md` Q46.
type MidiEventGraph =
  | {
      type: "noteOn";
      channel: Node<"i32">;
...
defineProcessor((ctx) => {
  ...
  return {
    process: () => {
      ...
      midiIn.onEvent("noteOn", ({ note, atSample }) => {
        noteState.write(note);
        trigOffset.write(atSample);
      });

      // Per-sample iteration triggers the envelope at sample-offset i == trigOffset.
      forSample((i) => {
        const fire = i.eq(trigOffset.read());
...
`event.midi({ to: 'main' })` returns a handle whose only emission primitive is the `emitIf` **method**: `midiOut.emitIf(condition, event)`. There is no plain `emit(event)` — `emitIf` is the single emission primitive across every audio-thread expression context: `forSample` / `forSample.byN` callbacks, `everyNSamples` callbacks (= delivered as the second argument of the surrounding `forSample` callback per Q43, not a free function), MIDI / event handler bodies, and the per-block top level (statements in the `process` body outside any `forSample`). See `decisions-log.md` Q4-b for the footgun reasoning, Q32-a for the cross-context unification. `emitIf` is dispatched off the handle (no free-function form); the same `handle.emitIf(cond, payload)` shape is used by generic `event<T>({ to: 'main' })` declarations (see `01-dsl.md` §4).
...
**Newly-constructed sysex** uses a build-time-fixed `Buffer<'u8'>` (= `state.buffer.u8(...)` declaration, see `01-dsl.md` §3.2). The author writes bytes into the buffer with `buf.write(idx, byteValue)` and passes the buffer plus a `length: Node<'i32'>` to `emitIf`. The framework ships `data[0 .. length-1]` to main:

```typescript
defineProcessor((ctx) => {
  const midiOut = event.midi({ to: "main", name: "midiOut" });
  const sysexBuf = state.buffer.u8({ size: 64 }); // build-time-fixed
  ...
  return {
    process: () => {
      ...
      forSample((i) => {
        const cond = i.eq(sendAt.read()); // sample-edge: fires exactly on the offset stored by the handler
        midiOut.emitIf(cond, {
          type: "sysex",
          data: sysexBuf, // Buffer<'u8'> reference
          length: txLen.read(), // Node<'i32'> — bytes to ship
          atSample: i,
        });
      });
    },
...
`<name>` matches the `name` passed to the worklet-side `event.midi({ from: 'main', name })` declaration. The same namespaced shape applies to every `event.midi({ from: 'main' })` declaration; multi-port processors expose one sub-surface per declared port (Q40, `decisions-log.md`).
...
For processors that declared `event.midi({ to: 'main', name })`, the main thread subscribes per port via `node.midi.<name>.onEvent(type, handler)`. Typical use is to forward emitted MIDI to a Web MIDI output device or to application logic. Diagnostics are exposed at `node.midi.<name>.diagnostics.overflowCount()`.
...
- **SAB available** (default): events flow through a `SharedArrayBuffer`-backed ring buffer with `Atomics`-based head / tail pointers. Sample-accurate timing is preserved end-to-end. The ring buffer **header layout is shared with the generic `event<T>` family (both directions)** (`02-messaging.md` §5.5 — `[head: i32][tail: i32][overflowCount: i32]` followed by the slot array); only the slot encoding differs (§4.1 above). MIDI in / out ports each get their own header + slot array; the sysex content buffer (§4.3) is allocated alongside.
...
```typescript
const midiIn = event.midi({ from: "main", name: "midiIn" }); // capacity: CAPACITY_256 (default)
const heavy = event.midi({ from: "main", name: "heavy", capacity: CAPACITY_1024 }); // override
```
...
- A monotonic `overflowCount` counter is incremented on each drop, exposed on the main thread as `node.midi.<name>.diagnostics.overflowCount()` (uniform with `node.events.<name>.diagnostics` for the `event<T>` family in both directions; see Q47).
