...
The build-time pipeline that turns a `defineProcessor` definition into the artifacts consumed by the worklet runtime, the main-thread client, and the offline runtime (`@unworklet/offline`). The pipeline is exposed as a **single async named export on `@unworklet/core`** — `compile(processor)`, returning `{ wasm, graph, memory, diagnostics, schemaHash }` in one call. All consumers — `@unworklet/unplugin` (= build pipeline integration), `@unworklet/offline` (= `renderOffline` invokes it internally), `replaceProcessor` (= live coding / hot swap), pure Node scripts, and browser host scripts that build processors at runtime (= visual programming editors, modular synth web apps, on-the-fly source evaluation) — call this same `compile` function. The compile entry point itself is the public surface; the internal stage modules (graph capture, static analysis, WASM emission, etc.) remain encapsulated inside `@unworklet/core`. Concrete TS generic constraint shape is impl-phase fill.

The `binaryen` toolkit (= the WASM emission backend, §4) is loaded via **dynamic import** from inside `compile`, so consumers that never call `compile` (= production runtimes that load a precompiled WASM binary emitted at build time) do not pay the `binaryen` bundle cost in their production bundle. `@unworklet/core` lists `binaryen` in its dependencies, but the dynamic-import boundary keeps it out of bundles that exercise only the runtime path (see `09-repo-structure.md` §2.4 for the package dependency layout).
...
Each invocation of `compile(processor)` runs 5 stages end-to-end for one `defineProcessor` definition. `@unworklet/unplugin` calls it per source file during the Vite build, `@unworklet/offline`'s `renderOffline` calls it on demand when no prebuilt binary is supplied, and any other host (= pure Node script, browser host that builds processors at runtime) calls it the same way.
...
4. **Worklet JS codegen** (§5) — emit the `AudioWorkletProcessor` subclass that wraps the WASM and marshals per-block I/O (see `04-worklet-runtime.md`), and expose the same machinery as the `CompiledProcessor.worklet = { initialize, process, parameterDescriptors }` function namespace so authors can build their own `class extends AudioWorkletProcessor` (escape hatch — `01-dsl.md` §11, Q80).
5. **Client TS codegen** (§6) — emit the typed `.d.ts` for `UnworkletNode<C>`, plus build-time metadata artifacts emitted by `@unworklet/unplugin`: `dist/<processor>.graph.json` (= AST DAG view), `.memory.json` (= per-declaration byte counts), `.diagnostics.json` (= 3-layer error / warning list with stable IDs), `.schema-hash.json` (= snapshot migration anchor). See `07-unplugin.md` §6.3 for the artifact contract.

Source maps thread `.ts` → AST → `.wasm` positions across all 5 stages and are emitted as a sidecar `.wasm.map` by `@unworklet/unplugin` (§7 + Q23+Q24+Q25). Per-section detail of stages §3 / §4 / §5 / §6 / §7 / §8 is impl-phase fill per Q61.
...
1. **Declaration scope** — the top of the `defineProcessor` body, before `return { process: ... }`. New `state.*`, `state.buffer.*`, `param.*`, `audioInput`, `audioOutput`, `event` (with `from` / `to` direction, plus `event.midi` ports), and `instantiate(...)` instantiations are recorded as graph slots. (`defineSubgraph` itself is the module-level subgraph constructor — it produces a subgraph value at module scope; only `instantiate(subgraph, ...args)` calls inside the processor body create per-processor instances.) Each declaration registers a `name` (when supplied) for later snapshot identity (see `01-dsl.md` §8).
...
| ---------------------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `scope-violation`                  | 2     | a declaration call (`state.*` / `state.buffer.*` / `param.*` / `audioInput` / `audioOutput` / `event` (incl. `event.midi`) / `instantiate(...)`) appears inside expression scope (= `process` body, `forSample` callback, handler body, or L1 helper)                                               | Q22-c       |
...
| `subgraph-missing-name`            | 2     | a `instantiate(subgraph, ...args)` call omits the instance `name` while the subgraph declares a named-factory slot (`.named(...)` / `.expose({ name })` / `persistent` / `publish`) — the slot's snapshot / main-side path would otherwise be `__sg_N/...`, dependent on instantiation order (§8.1) | Q41         |
...
<!-- Emission target (binaryen.js or custom emitter). The chosen backend is
     loaded via dynamic import inside `compile` on `@unworklet/core` (see §1
     prose), so consumers that only run precompiled
